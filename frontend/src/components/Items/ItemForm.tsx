import {
  ...
  import { type ApiError, type ItemCreate, type ItemPublic, type ItemUpdate, ItemsService } from "../../client";
  ...
  const ItemForm = ({ item, mode, isOpen, onClose }: ItemFormProps) => {
    ...
    const mutation = useMutation({
      mutationFn: (data: ItemCreate | ItemUpdate) => isEdit ?
        ItemsService.updateItem({ id: item!.id, requestBody: data }) :
        ItemsService.createItem({ requestBody: data }),
      onSuccess: () => {
        showToast("Success!", `Item ${isEdit ? "updated" : "created"} successfully.`, "success");
        reset();
        onClose();
      },
